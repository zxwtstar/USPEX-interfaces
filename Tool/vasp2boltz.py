...
def write_intrans_boltztrap(boltzdir='./',filename='hte.intrans',
                            boltz_generic=True, E_Fermi=0.0, n_electrons=1.0,
                            efcut=0.15, Tmax=800.0, Tdelta=50.0):
    parentdir=os.getcwd()
    if not os.path.isdir(boltzdir):
        os.mkdir(boltzdir)
    os.chdir(boltzdir)
    intrans_tmpl = """{mode}        # {mode_comment}
0 0 0 0.0         # iskip (not presently used) idebug setgap shiftgap
{E_Fermi:7.5f} 0.0005 0.4 {n_electrons:6.1f} # Fermilevel (Ry), energygrid, energy span around Fermilevel, number of electrons
...
{efcut:5g}                     # (efcut) energy range of chemical potential
{Tmax:5g} {Tdelta:5g}               # Tmax, temperature grid
-1.                       # energyrange of bands given individual DOS output sig_xxx and dos_xxx (xxx is band number)
HISTO
"""

    if boltz_generic:
        mode, mode_comment = "GENE", "use generic interface"
    else:
        mode, mode_comment = "WIEN", "use wien interface"
    contents = intrans_tmpl.format(mode=mode, mode_comment=mode_comment,
                                   E_Fermi=E_Fermi, n_electrons=n_electrons,
                                   efcut=efcut, Tmax=Tmax, Tdelta=Tdelta)
    fout=open(filename,'w')
    fout.write(contents)
    fout.close()
    os.chdir(parentdir)
...
"""
    ...
    proc.stdin.write(str.encode(sym_in))
    proc.stdin.write(str.encode(coords_lines))
    sym_out, _ = proc.communicate()

    output = StringIO(sym_out.decode("utf-8"))
...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(prog="vasp2boltz")
    parser.add_argument('--casename', '-n', dest='casename',
                        type=str, required=True,
                        help="The casename of BoltzTraP files.")
    parser.add_argument('--max-temperature', '-t',
                        dest='Tmax', type=float, default=800.0,
                        help="The maximum temperature for evaluation.")
    parser.add_argument('--delta-temperature', '-d',
                        dest='Tdelta', type=float, default=50.0,
                        help="The delta of the temperature grid.")
    parser.add_argument('--efcut', '-c',
                        dest='efcut', type=float, default=0.15,
                        help="energy range of chemical potential.")
