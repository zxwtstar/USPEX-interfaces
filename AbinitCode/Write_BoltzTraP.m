...
% unixCmd('cp ../Property/w_struct .');
% unixCmd('cp ../Property/BoltzTraP .');
...
unixCmd('cp CONTCAR_old CONTCAR'); % Required by Read_VASP_Structure
unixCmd('cp OUTCAR_old OUTCAR');   
unixCmd('cp OSZICAR_old OSZICAR'); % Recover OSZICAR for Read_VASP
% unixCmd('cp POSCAR_old POSCAR');

% delete previous results
  casename = [dir '-' num2str(Ind_No)];
  % delete([casename '.*']);  %% DANGEROUS
  ...
  USPEXPATH = strtrim(USPEXPATH);
  python_uspex([USPEXPATH '/FunctionFolder/Tool/vasp2boltz.py'], ...
               ['--casename ' casename], ...
               ['--max-temperature ' num2str(ORG_STRUC.TEparam.Tmax)], ...
               ['--delta-temperature ' num2str(ORG_STRUC.TEparam.Tdelta)], ...
               ['--efcut ' num2str(ORG_STRUC.TEparam.efcut)]);

% Remove the copied files
unixCmd('rm OUTCAR'); % Delete the recovered files
  
% % write CalcFoldX.struct
% % fortran code 'w_struct' takes care all
% unixCmd(['$USPEXPATH/FunctionFolder/Tool/w_struct > ' dir '.struct']);
