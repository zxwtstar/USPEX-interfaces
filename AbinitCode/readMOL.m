...
if code == 2  %FOR SIESTA, we read Zmatrix
   ...
   if code0 == 6  % for DMACRYS: the order of atoms in the molecule differes from what we give as input

       for molNum = 1 : length(ORG_STRUC.numMols)
           TypeX = ORG_STRUC.STDMOL(molNum).types;
	   newTypeX = [];
           while length(TypeX) > 0
	       RM = find(TypeX == TypeX(1));
	       newTypeX = [newTypeX ; TypeX(RM)];
               TypeX(RM) = [];
           end
           molTypeList(molNum).type = newTypeX;
       end
       newType = [];
       for NmL = 1 :length(numMols)
	   for Xmol = 1 : numMols(NmL)
                newType = [newType ; molTypeList(NmL).type]; 
           end
       end
       typesAList = ORG_STRUC.atomType(newType)';
