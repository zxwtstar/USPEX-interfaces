...
unixCmd(['echo ' num2str(lat(1,:)) ' > structure.dat']);
unixCmd(['echo ' num2str(lat(2,:)) ' >> structure.dat']);
unixCmd(['echo ' num2str(lat(3,:)) ' >> structure.dat']);

for i = 1:L
  unixCmd(['echo ' num2str(POP_STRUC.POPULATION(Ind_No).COORDINATES(i,:)*lat) ' >> structure.dat']);
end
