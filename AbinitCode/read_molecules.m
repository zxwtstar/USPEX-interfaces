...
%%%%%%%%%%%%%%%%%%%%%%%%%%%   Zmatrix  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       elseif num > 1 %orientate the molecule according to the priciple Rotation axis
          ...
          MOLCOORS = bsxfun(@minus, MOLCOORS, mean(MOLCOORS)); % Matrix-
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       status = fclose(handle);
        ...
        unixCmd(['echo For Molecules, you did not give enough MOL_x files >>' ORG_STRUC.resFolder '/' ORG_STRUC.log_file]);
        unixCmd(['echo The program is stopping......... >>' ORG_STRUC.resFolder '/' ORG_STRUC.log_file]);
        quit
    end
end
...
try
    ...
    if ~isempty(CenterminDistMatrice)
      if size(str2num(CenterminDistMatrice)) ~= [N_Mols, N_Mols]
