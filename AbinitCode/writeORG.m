...
if ORG_STRUC.dimension == -2
	...
	'',...
        'Wang Z., Zhou X.-F., Zhang X., Zhu Q.,Dong H., Zhao M., Oganov A.R. (2015)', ...
        'Phagraphene: A Low-Energy Graphene Allotrope Composed of', ...
        '5-6-7 Carbon Rings with Distorted Dirac Cones.', ...
        'Nano Lett. 15, 6182' ...
        };
    formatted_rows = createHeader_wrap(text, 'left');
    for i=1:size(formatted_rows,2)
...
if ORG_STRUC.dimension == 3
    fprintf(fp,[alignLine('Random topological structure generator') '\n']);
    text = {'Bushlanov P.V., Blatov V.A., Oganov A.R. (2019).' ...
...
if length(ORG_STRUC.optType) > 1
    fprintf(fp,[alignLine('Multi-Objective Pareto optimization') '\n']);
    text = {'Allahyari Z., Oganov A.R. (2020).' ...
        'Multi-objective Optimization as a Tool for Material Design.' ...
        'In: Andreoni W., Yip S. (eds) Handbook of Materials Modeling.' ... 
        'Springer, Cham, pp 2777-2790. doi.org/10.1007/978-3-319-44680-6_71' ...
        };
    formatted_rows = createHeader_wrap(text, 'left');
    for i=1:size(formatted_rows,2)
        fprintf(fp,[formatted_rows{i} '\n']);
...
if ORG_STRUC.molecule == 1
    ...
    for i = 1 : size(ORG_STRUC.numIons, 2)
        ...
        for j = 1 : length(STDMOL.types)
            ...
            Zmatrix(j,2) = 180/pi * Zmatrix(j,2);
            Zmatrix(j,3) = 180/pi * Zmatrix(j,3);
...
elseif ORG_STRUC.platform == 1
    fprintf(fp,'The script for job submission is prepared seperately in Submission/*_local.py\n');
elseif ORG_STRUC.platform == 2
    fprintf(fp,'The script for job submission is prepared seperately in Submission/*_remote.py\n');
