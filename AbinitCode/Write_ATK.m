...
unixCmd('echo "# Set up lattice" > ATK.in');
unixCmd(['echo vector_a = [' num2str(lat(1,1)) ', ' num2str(lat(1,2)) ', ' num2str(lat(1,3)) ']*Angstrom >> ATK.in']);
unixCmd(['echo vector_b = [' num2str(lat(2,1)) ', ' num2str(lat(2,2)) ', ' num2str(lat(2,3)) ']*Angstrom >> ATK.in']);
unixCmd(['echo vector_c = [' num2str(lat(3,1)) ', ' num2str(lat(3,2)) ', ' num2str(lat(3,3)) ']*Angstrom >> ATK.in']);
unixCmd(['echo "lattice = UnitCell(vector_a, vector_b, vector_c)" >> ATK.in']);
unixCmd('echo "# Define elements" >> ATK.in');
s = [];
...
unixCmd(['echo "elements = [ ' s ']" >> ATK.in']);

unixCmd('echo "# Define coordinates" >> ATK.in');
for i = 1 : sum(numIons)
  prefixS = '';
  suffixS = ',';
  if i == 1
  ...
  unixCmd(['echo ' prefixS '[' num2str(coord(i,1)) ', ' num2str(coord(i,2)) ', ' num2str(coord(i,3)) ']' suffixS ' >> ATK.in']);
end

unixCmd(['echo "# Set up configuration" >> ATK.in']);
unixCmd(['echo "bulk_configuration = BulkConfiguration(" >> ATK.in']);
unixCmd(['echo "bravais_lattice=lattice," >> ATK.in']);
unixCmd(['echo "elements=elements," >> ATK.in']);
unixCmd(['echo "cartesian_coordinates=cartesian_coordinates" >> ATK.in']);
unixCmd(['echo ")" >> ATK.in']);
...
try
 ...
 status = fclose(handle1);
    ...
    unixCmd(['echo "ATK input is not present for Step ' num2str(Step) '" >> ERROR_ATK']);
    quit
end
