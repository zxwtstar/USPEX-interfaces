function Write_GULP_surface(Ind_No)
...
unixCmd(['echo svectors >> input']);

unixCmd(['echo ' num2str(Lattice(1,1)) '  ' num2str(LATTICE(1,2)) ' >> input']);
unixCmd(['echo ' num2str(Lattice(2,1)) '  ' num2str(LATTICE(2,2)) ' >> input']);

unixCmd(['echo ''cartesian region 1'' >> input']);
item=0;

for i=1:size(COORDINATES,1)
  if POP_STRUC.POPULATION(Ind_No).chanAList(i)==1
     unixCmd(['echo ' num2str(POP_STRUC.POPULATION(Ind_No).typesAList(i)) ' ' num2str(COORDINATES(i,:)) '  >> input']);
     item=item+1;
     POP_STRUC.POPULATION(Ind_No).Index(item)=i;
  end
end

unixCmd(['echo ''cartesian region 2'' >> input']);
for i=1:size(COORDINATES,1)
   if POP_STRUC.POPULATION(Ind_No).chanAList(i)<1
      unixCmd(['echo ' num2str(POP_STRUC.POPULATION(Ind_No).typesAList(i)) ' ' num2str(COORDINATES(i,:)) '  >> input']);
      item=item+1;
