...
if exist('relaxation_restart_file.FHIaims')
   delete('relaxation_restart_file.FHIaims');
end
if exist('geometry.in.next_step')
   %unixCmd('mv geometry.in.next_step geometry_old');
end

try
   unixCmd(['cat aims_control_' num2str(Step) ' > control.in']);
catch
...
if ORG_STRUC.dimension ~= 0
    unixCmd(['echo k_grid ' num2str(Kpoints) ' >> control.in']);
end
